...
const StaffManagement = () => {
  ...
  const handleToggleActive = async (staffId, currentStatus) => {
    try {
      await API.put(`/staff/${staffId}`, { is_active: !currentStatus });
      setSuccess(
      ...
      <div className="staff-wrapper">
        <div className="staff-header">
          <h2 style={{ display: "flex", alignItems: "center", gap: 9 }}>
            <IconUsers size={22} color="currentColor" />
            Staff Management
          </h2>
          <button
          ...
          >
            {showForm ? (
              ...
              <>
                <IconPlus size={14} color="currentColor" /> Add Staff
              </>
            )}
          </button>
        </div>
              ...
              <tbody>
                {staffList.map((staff) => (
                  <tr key={staff.staff_id}>
                    ...
                    <td>
                      <button
                        className={`btn-toggle ${staff.is_active ? "btn-deactivate" : "btn-activate"}`}
                        onClick={() =>
